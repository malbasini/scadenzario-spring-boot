...
public class PayPalService {
    ...
    public Subscription executePayment(String paymentId, String payerId, HttpSession session) throws PayPalRESTException {
        ...
        if ("approved".equalsIgnoreCase(executedPayment.getState())) {
            ...
            int scadenzaId = (int) session.getAttribute("scadenzaId");
            if (session.getAttribute("userId") == null || session.getAttribute("scadenzaId") == null) {
                throw new IllegalStateException("I dettagli della sessione non sono stati trovati.");
            }
            Subscription subscription = subscriptionService.createSubscriptionPayPal(userId, scadenzaId, paidAmount, currency, "PAYPAL", executedPayment.getId());
            return subscription;
        }
